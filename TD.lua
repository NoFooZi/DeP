      ...
      if msg.content._ == "messageText" then
        ...
        if is_sudo(msg) then
          ...
          elseif text:match("^([Ii]nfo)$") or text:match("^([Pp]anel)$") then
            ...
            local msgadd = redis:get("tg:" .. Ads_id .. ":addmsg") and "ση" or "ŐŦŦ"
            local numadd = redis:get("tg:" .. Ads_id .. ":addcontact") and "ση" or "ŐŦŦ"
            local txtadd = redis:get("tg:" .. Ads_id .. ":addmsgtext") or "addi"
            local autoanswer = redis:get("tg:" .. Ads_id .. ":autoanswer") and "ση" or "ŐŦŦ"
            local wlinks = redis:scard("tg:" .. Ads_id .. ":wait")
            local glinks = redis:scard("tg:" .. Ads_id .. ":good")
            local links = redis:scard("tg:" .. Ads_id .. ":save")
            local offjoin = redis:get("tg:" .. Ads_id .. ":offjoin") and "ŐŦŦ" or "ση"
            local offlink = redis:get("tg:" .. Ads_id .. ":offlink") and "ŐŦŦ" or "ση"
            local openjoin = redis:get("tg:" .. Ads_id .. ":openjoin") and "ση" or "ŐŦŦ"
            ...
            local nlink = redis:get("tg:" .. Ads_id .. ":link") and "ση" or "ŐŦŦ"
            local contacts = redis:get("tg:" .. Ads_id .. ":savecontacts") and "ση" or "ŐŦŦ"
            local fwd = redis:get("tg:" .. Ads_id .. ":fwdtime") and "ση" or "ŐŦŦ"
            local max_i = redis:get("tg:" .. Ads_id .. ":sendmax") or 5
            local delay = redis:get("tg:" .. Ads_id .. ":senddelay") or 2
...
ADD MSG ]] .. tostring(msgadd) .. [[

MSG FOR ADD CONTACT ]] .. tostring(txtadd) .. [[
]]
          ...
          elseif text:match("^([Ss]leep) (%d+)$") then
            local matches = text:match("%d+")
            send(msg.chat_id, msg.id, "OK GOOD BY")
            os.execute("sleep " .. tonumber(math.floor(matches) * 60))
            return send(msg.chat_id, msg.id, "HELLP MY Babe")
          elseif text:match("^([Hh]elp)$") then
            local txt = [[
••Info••
برای  با خبر شدن از امار و عملکرد ربات
••Promote (user-Id)••
افزودن مدیر شناسه
••Demote (userId)••
برداشتن مدیر شناسه
...
فوروارد پیام مد نظر به همه یا سوپرگروه ها یا گپ ها یا پیوی ها با ریپلی
••AddedMsg (on or off)••
پیام افزودن مخاطب فعال یا خاموش
••SetAddedMsg (text)••
تنظیم متن افزودن مخاطب متن
••AddToAll @(usename)••
افزودن به تمامی گروه ها و سوپر گروه ها یوزر نیم
••AddMembers••
افزودن تمام ممبر های اکانت
••Join (on or off)••
عضویت  روشن یا خاموش
••ChkLnk (on or off)••
شناسایی لینک روشن یا خاموش
••FindLnk (on or off)••
پیدا کردن لینک روشن یا خاموش
••Add (phone number)••
افزودن مخاطب {با ریپلی روی شماره مد نظر}
••AddContact (on or off)••
افزودن مخاطب روشن یا خاموش
••Refresh••
تازه سازی
••Join (Private Link)••
عضو شدن به لینک مد نظر
••Ping••
اگاهی از انلاینی و یا افلاینی
]]
            return send(msg.chat_id, msg.id, txt)
            elseif text:match("^([Cc]reator)$") then
local txt = [[
•••<b>Creator</b>: @DeP_DeV•••
—-------------------------------------------------------------------------
─═हई <b>Channel creator</b> : @NoFooZiTM  ईह═─
]]
return send(msg.chat_id_,msg.id_, txt)
elseif text:match("^([Aa]bout)$") then
local txt = [[
<b>VERSION</b>: 2.0
<b>TDBOT</b>
<b>ADDER AND JOINER</b>
<b>BY SAMIYAR-BANG @DeP_DeV</b>
]]
return send(msg.chat_id_,msg.id_, txt)
